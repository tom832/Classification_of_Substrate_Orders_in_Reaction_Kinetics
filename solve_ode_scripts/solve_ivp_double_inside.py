...
def mechanism(t, z, A, a, B, C, c):
    s, p, bp, cat, cats= z
    return (a*cats-A*cat*s-B*cats*s, B*cats*s, C*cats-c*cat*bp, -A*cat*s+a*cats+B*cats*s+C*cats-c*cat*bp, A*cat*s-a*cats-B*cats*s-C*cats+c*cat*bp)
...
if __name__ == '__main__':
    ...
    # python solve_ivp_double_inside.py --task_name 0 --task_id 0 --job_num 100000 --num_proc 5
    ...
    tasks = []
    save_root = os.path.join('../data', 'ode_raw_data', f'{task_name}__double_inside')
    # save_root = f'{task_name}__double_inside__{task_id}'
    ...
    with ProcessPoolExecutor(max_workers=num_proc) as executor:
        for _ in range(job_num):
            ...
            if task_name == '1':
                a = random.uniform(1, 10)*10**random.choice([-5, -4, -3, -2, -1, 0, 1, 2, 3, 4])
                C = random.uniform(10**(-5), B)
                while (a/(A+B) < 10) or (A*B/(a+C) > 15) or (A*B/(a+C) < 0.5):
                    A = random.uniform(1, 10)*10**random.choice([-5, -4, -3, -2, -1, 0, 1, 2, 3, 4])
                    B = random.uniform(1, 10)*10**random.choice([-5, -4, -3, -2, -1, 0, 1, 2, 3, 4])
                    a = random.uniform(1, 10)*10**random.choice([-5, -4, -3, -2, -1, 0, 1, 2, 3, 4])
                    C = random.uniform(10**(-5), B)
